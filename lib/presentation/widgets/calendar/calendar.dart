...
class _CalendarWidgetState extends State<CalendarWidget> {
  ...
  late DateTime _focusedDay =
      widget.showFocusedDay == false ? DateTime.now() : widget.focusedDay;
  ...
  selectData() {
    widget.dataSelectDate
        ?.map((data) => _selectedDays.add(DateTime.parse(data.date)))
        .toList();
  }
  ...
  Widget toDay(context, date, event) => Container(
        ...
        decoration: BoxDecoration(
            color: _checkToday == true
                ? AppColors.light().mainDarkGreen.withOpacity(0.2)
                : Colors.transparent,
  ...
  Widget dayDefault(context, datetime, focusedDay, AppColors colors) =>
      Container(
        width: 40,
        height: 40,
        decoration: BoxDecoration(
  ...
  Widget selectDay(context, datetime, focusedDay) {
    if (widget.disableSelectDate == false) {
      final selectedDay = datetime.day.toString() +
      ...
      Color? handleColor() {
        ...
        if (widget.dataSelectDate != null) {
          for (int a = 0; a < widget.dataSelectDate!.length; a++) {
            var dateDay = DateTime.parse(widget.dataSelectDate![a].date)
                    .day
                    .toString() +
                DateTime.parse(widget.dataSelectDate![a].date)
                    .month
                    .toString() +
  ...
  handleEnabledDay(date) {
    ...
    if (widget.dataDisable != null) {
      for (int i = 0; i < widget.dataDisable!.length; i++) {
        String disable =
            DateTime.parse(widget.dataDisable![i].date).day.toString() +
        ...
        child: TableCalendar(
          ...
          calendarStyle: CalendarStyle(
            todayDecoration: BoxDecoration(
              color: appColors.mainDarkGreen.withOpacity(0.16),
          ...
          onDaySelected: widget.disableSelectDate == false
              ? _onDaySelected
              : _onDaySelectedOne,

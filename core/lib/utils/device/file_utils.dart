...
abstract class AppFileTypeCore {
  // Image
...
class FileUtilsCore {
  ...
  bool get isImage {
    final type = fileType;
    return type == AppFileTypeCore.jpg ||
        type == AppFileTypeCore.jpeg ||
        type == AppFileTypeCore.png;
  }

  bool get isPDF {
    final type = fileType;
    return type == AppFileTypeCore.pdf;
  }

  bool get isDoc {
    final type = fileType;
    return type == AppFileTypeCore.doc || type == AppFileTypeCore.docx;
  }

  bool get isExcel {
    final type = fileType;
    return type == AppFileTypeCore.xls || type == AppFileTypeCore.xlsx;
  }
  ...
  static Future<dynamic> getImageSecond({
    PickerImageType type = PickerImageType.camera,
    required BuildContext context,
    int maxAssets = 5,
  }) async {
    final picker = ImagePicker();
    DeviceUtilsCore.dismissKeyboard();
    String path = '';
    ...
    if (type == PickerImageType.camera) {
      ...
      if (!StringUtilsCore.isNullOrEmpty(image)) {
        path = image?.path ?? '';
      }
      ...
      if (!StringUtilsCore.isNullOrEmpty(images)) {
        final File? file = await images![0].file;
        if (!StringUtilsCore.isNullOrEmpty(file)) {
          path = file?.path ?? '';
        }
      }
  ...
  static Future<List<File>> getFiles() async {
    final result = await FilePicker.platform.pickFiles(
      ...
      allowedExtensions: [
        AppFileTypeCore.jpg,
        AppFileTypeCore.jpeg,
        AppFileTypeCore.png,
        AppFileTypeCore.pdf,
        AppFileTypeCore.doc,
        AppFileTypeCore.docx,
        AppFileTypeCore.xls,
        AppFileTypeCore.xlsx,
      ],
    );
